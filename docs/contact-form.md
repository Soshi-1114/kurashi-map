...
| 表示ラベル | name | 種別 | 必須 | 制限 |
|---|---|---|---|---|
| お問い合わせの種類 | `category` | select | ✓ | 後述の 5 種のいずれか |
| お名前（ニックネーム可） | `name` | text | – | 50 文字 |
| メールアドレス | `email` | email | – | 254 文字／返信を希望する場合のみ記入 |
...
| value | 表示 |
|---|---|
| `data-error` | 掲載データの誤り・更新のご指摘 |
| `feature` | 機能のご要望 |
| `bug` | 不具合のご報告 |
| `media` | 取材・掲載・提携のご相談 |
| `other` | その他 |

**メールアドレスを任意にする理由**: 誤りの指摘は「教えて終わり」で構わない利用者が多く、必須にすると指摘そのものが減る。返信が必要なケースだけ利用者が判断できるよう、ラベルで「返信をご希望の場合は必ずご記入ください」と明示する。
...
```ts
export const CONTACT_CATEGORIES = [...] as const;
