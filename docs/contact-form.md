...
構成は独立した 3 つの軸で決まる。**軸ごとに選び、あとから軸単位で差し替えられる形にしておく**のが要点（例: 配信サービスを変えても `lib/mailer.ts` の中だけで済む）。

#### 軸① 受け口 — フォーム送信をどこで受けるか

| | 方式 | 実装量 | 評価 |
|---|---|---|---|
| **A** | **Next Route Handler（`POST /api/contact`）** | 中 | **推奨**。既存の `/api/muni` `/api/shelters` と同じ書き方。`Request` を渡すだけで単体テストでき、CI に載る。既存 `middleware.ts` の JP 限定 403 がそのまま効く。CSP 変更なし |
| B | Next Server Action | 中 | `<form action={fn}>` で **JS 無効でも送信できる**のが唯一の実利。ただし Route Handler より単体テストが書きにくく、`useActionState` 前提でフォーム実装も変わる。地図が主機能で JS 必須のサイトなので、この利点は活きにくい |
| C | 外部フォーム SaaS へ直接 POST（Formspree / Web3Forms / Getform） | **小** | サーバー実装ゼロ。最短で公開できる。代償として CSP の `connect-src` に外部ドメイン追加、送信内容が第三者を経由、無料枠 50〜250 通/月、スパム対策・文面整形が SaaS 任せ。自前実装・依存最小という本リポジトリの方針からは外れる |
| D | 別ホストの関数（Cloudflare Workers / Supabase Edge Functions） | 大 | Vercel の関数実行を避けられるが、デプロイ先とシークレット管理が二重になる。この規模で管理対象を増やす理由がない |
| E | `mailto:` リンクのみ（フォームなし） | 極小 | メールアドレスがソースに露出しスパム収集の的になる。利用者側もメーラー起動が要る。**却下** |

#### 軸② 配信 — 運営者にどう届けるか

| 方式 | 到達性 | 評価 |
|---|---|---|
| **メール配信 API を `fetch`（Resend 等）** | 高 | **推奨**。SDK 不要で npm 依存ゼロ。詳細と他社比較は §6.1 |
| SMTP（nodemailer + Gmail 等） | 中〜高 | nodemailer への依存が増える。サーバーレスでの SMTP はコールドスタートとタイムアウトの調査コストが高い |
| チャット Webhook（Discord / Slack） | – | 即時に手元へ届き、無料で、スパムが来ても痛くない。ただし**返信が手作業**になり「メールで届く」という要件を満たさない。メールと**併用**する形なら有効（下記パターン 2） |

#### 軸③ 保管 — 送信内容を残すか

| 方式 | 評価 |
|---|---|
| **残さない（メールのみ）** | **推奨**。受信箱が保管場所を兼ねる。個人情報を自前で持たないので、プライバシーポリシーの記述も運用も最小で済む |
| GitHub Issues 化 | データ誤りの指摘だけなら公開トラッカーとして機能し透明性も高い。ただし利用者の氏名・メールが public repo に載るため**そのままでは不可**。公開する場合は本文のみを運営者が手で起票する運用に留める |
| Google Sheets / Notion / Vercel KV | 件数が増えて集計したくなってから。最初から入れると個人情報の保管先が増えるだけ |

### 1.2 組み合わせた構成パターン

| | 構成 | 実装量 | npm 依存 | 月額 | CSP 変更 | 拡張余地 |
|---|---|---|---|---|---|---|
| **1** | **Route Handler + Resend（保管なし）** | 中 | なし | 0 円 | 不要 | 高 |
| 2 | パターン 1 + Discord Webhook 通知を併用 | 中+小 | なし | 0 円 | 不要 | 高 |
| 3 | 外部フォーム SaaS へ直接 POST | 小 | なし | 0 円（枠超で有料） | **必要** | 低 |
| 4 | Server Action + Resend | 中 | なし | 0 円 | 不要 | 高 |

**パターン 1 を採用する。** 理由は 3 点。

1. **既存の作法にそのまま乗る。** Route Handler・`fetch`・純関数＋vitest という、このリポジトリに既にある形だけで完結し、新しい概念も npm 依存も増えない。
2. **防御が二重にかかる。** `middleware.ts` の JP 限定 403 がフォーム到達前に効き、CSP は現状のまま（外部ドメイン追加が不要）。パターン 3 は両方を緩めることになる。
3. **後から足せる。** Turnstile・レート制限の外部ストア化・保管先の追加・配信サービスの乗り換えが、いずれも自前コードの内側で完結する。パターン 3 は SaaS の機能範囲が上限になる。

パターン 2（Discord への同時通知）は**パターン 1 の上に後から 10 行程度で足せる**ので、最初から入れない。運用してみて「メールだと気づくのが遅い」となった時点で `lib/mailer.ts` に通知先を 1 つ増やす。

**パターン 3 を選ぶとしたら**、「まず窓口を最速で開けて、需要があるか見てから作り込む」という判断のとき。その場合も `/contact` ページと `ContactForm` は同じものを使い、送信先 URL だけを差し替える形にしておけば、後からパターン 1 へ移行できる。
...
問い合わせは多くて 1 日数件の想定なので、**無料枠の大小より「設定の少なさ」と「依存を増やさないこと」で選ぶ**。

| 方式 | 依存追加 | 無料枠の目安 | 評価 |
|---|---|---|---|
| **Resend の REST API を `fetch`** | **なし** | 3,000 通/月・100 通/日 | **推奨**。`https://api.resend.com/emails` に JSON を POST するだけで SDK 不要。ドメイン検証は DNS レコードを数件足すだけで、設定項目がもっとも少ない |
| Brevo（旧 Sendinblue） | なし（REST 可） | 300 通/日 | 無料枠は最大級。管理画面がマーケティング機能中心で、単発の通知用途にはやや過剰 |
| SendGrid / Mailgun | なし（REST 可） | 縮小傾向・アカウント審査あり | 代替。審査と設定項目が多い |
| Amazon SES | なし（署名処理は自前） | 1,000 通あたり $0.10 | 最安だがサンドボックス解除の申請が要り、リクエスト署名（SigV4）を自前で書くことになる。この規模では割に合わない |
| Gmail SMTP + nodemailer | あり（nodemailer） | 実質無制限 | アプリパスワードの管理が必要。サーバーレスでの SMTP 接続はコールドスタートが重く、タイムアウトの調査コストが高い |
| Formspree / Google フォーム等 | なし | 50 通/月程度 | 受け口ごと外に出す方式（§1.1 の C）。デザイン統一・CSP・導線がちぐはぐになる |

→ **Resend + 素の `fetch`** を採用する。npm 依存はゼロのままで済む。乗り換えが必要になっても影響は `lib/mailer.ts` の内側に閉じる。

※ 各社の無料枠は改定されうるため、契約前に最新の条件を確認すること。
