...
|---|---|---|---|---|
| **annual** (`data-update-annual.yml`) | e-Stat / 国土数値情報 L01 / こども家庭庁 Excel | 人口・家賃・医療機関・地価・待機児童 | 年1回 3/15 04:00 JST | `ESTAT_APP_ID` |
| **quarterly** (`data-update-quarterly.yml`) | 不動産情報ライブラリ reinfolib | 災害リスク・生活インフラ（駅/保育） | 四半期 1/1・4/1・7/1・10/1 04:00 JST | `REINFOLIB_API_KEY` |
...
6. `fetch-waitlist.mjs --all`（待機児童, CFA）— Excel を1回パースして全県へ反映
7. 地価ループ（県別）: 各県の L01 zip を DL（3回リトライ）→ `fetch-land-price.mjs --pref=X`。
   県別の失敗は `::warning::` で記録し継続（他県は止めない）
8. `data/` に差分があれば単一 PR（ブランチ `data/annual-{run_id}`）を `gh` CLI で作成
...
2. `fetch`（県ごとの matrix, `max-parallel: 3` = reinfolib 過負荷防止）:
   ...
   - `fetch-amenities.mjs`（駅/保育・幼稚園, reinfolib XKT015/007）
     - **医療機関はここでは更新しない**: 原典の国土数値情報 P04 が令和2年度以降未更新のため、
       毎年公表される厚労省「医療施設調査」（e-Stat）から annual の `fetch-medical.mjs` が
       更新する（§2）。本スクリプトは既存の medicalFacilities を保持する。
     - amenities の表示ラベル（source/asOf）は `versions.mjs` の `AMENITIES_SOURCE` /
       `AMENITIES_ASOF` に集約（fetch-medical.mjs と同期）。reinfolib のデータ更新
       （毎年夏〜秋の報道発表）を確認したら「駅・保育 令和N年度」部分を見直す。
...
| 生活インフラ（駅・保育） | reinfolib XKT015/007 | 年度更新 | `fetch-amenities.mjs` |
