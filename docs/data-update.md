...
| **quarterly** (`data-update-quarterly.yml`) | 不動産情報ライブラリ reinfolib | 災害リスク・生活インフラ（駅/保育/医療） | 四半期想定（※下記 既知の注意点） | `REINFOLIB_API_KEY` |
...
2. `fetch`（県ごとの matrix, `max-parallel: 5`）:
   ...
   3. `fetch-population-*.mjs`（人口・国勢調査, e-Stat）※スクリプト名に注意（既知の注意点）
...
### スケジュール
- cron `0 19 31 12,3,6,9 *`（**意図**: 1/1・4/1・7/1・10/1 の 04:00 JST = 各前日
  19:00 UTC）。
- ⚠️ **既知の不具合**: 6月・9月は31日が存在しないため `day=31, month=6/9` は発火しない。
  実際に発火するのは **1/1（12/31 UTC）と 4/1（3/31 UTC）の年2回のみ**。
  7/1・10/1 ぶんは現状走らない（手動 `workflow_dispatch` で補う）。詳細は「既知の注意点」。
...
1. **annual の人口取得ステップのスクリプト名**
   ワークフローは `node scripts/fetch-population.mjs` を呼ぶが、リポジトリ上の実ファイルは
   `scripts/fetch-population-2025.mjs`。名称不一致のままだと当該ステップが
   「Cannot find module」で失敗し、**直列ゆえ同じ県の rent/land/waitlist も走らない**。
   → ワークフローの参照名を実ファイルに合わせるか、スクリプトを汎用名にリネームして解消する。

2. **quarterly cron が年2回しか発火しない**（前述）
   `0 19 31 12,3,6,9 *` は 6月/9月の31日が無く 7/1・10/1 が発火しない。
   → 例: `0 19 31 12,3 *` と `0 19 30 6,9 *` の2本に分割するなどで是正可能。
