...
3. `fetch-population-2025.mjs --all`（人口・国勢調査, e-Stat）— 全国の全コードを100件チャンクで
   まとめ取得（~19req/指標）し各県へ分配。人口総数のほか **5年間の人口増減率（%）を
   `populationChangeRate` として保存**（人口増加率ランキング /map/population-trend で使用。
   `populationTrend` はこの率の5区分）
...
#### 都道府県別の空き家率（`fetch-vacancy-pref.mjs`）

`fetch-vacancy.mjs`（市区町村別）とは**別に**、同じ統計表（statsDataId 0004021421）の
**都道府県行**を取得して `data/vacancy-pref.json` に保存するスクリプトがある。

理由: 同じ調査でも市区町村別集計は人口1.5万人未満の町村を含まない。それらは空き家率の
高い過疎地に偏るため、市区町村を合算すると系統的に低く出て、総務省公表の順位と食い違う
（徳島 21.33% / 和歌山 21.25% が 20.4% / 20.7% になり **1位が逆転**する）。
都道府県の値が必要な面（`/ranking/vacancy-high/prefecture`）では必ずこちらを使う。

実行は市区町村版と同じタイミングでよい（出典は5年周期）:

```bash
node --env-file=.env.local scripts/fetch-vacancy-pref.mjs
```

検証: 実行ログの全国合計が公表の 13.8% に一致すること（現在 13.84%）。
判断基準の一般形は `lib/prefRankings.ts` 冒頭のコメントを参照。
...
|---|---|---|---|
| 家賃 | 総務省 住宅・土地統計調査「家賃階級別の借家数」（e-Stat statsDataId 0004021470） | 5年ごと（次回2028年調査） | `fetch-rent.mjs`（`--all` 全国一括。**値は公表値そのものではなく、階級中点の加重平均による二次算出値**＝UI 表記は「家賃平均」。空き家率と同じ調査のため対象制約も同じ＝人口1.5万人未満の町村は集計対象外。`asOf` と statsDataId はスクリプト内リテラルで、調査更新時は `fetch-rent.mjs` を直接書き換える） |
...
1. **手動更新のバージョンを毎年忘れない**
   ...
   **`scripts/_lib/versions.mjs` の1箇所だけ**更新する
   ...
   人口は asOf フィールドを持たないため、H1 の鮮度ラベル **`POPULATION_FRESHNESS`**（現在
   「2025年国勢調査」）も国勢調査の年度更新時に同ファイルで書き換える（他指標のラベルは
   データの asOf から自動導出）。
...
```
GitHub Actions からも実行できる: **`indexnow.yml`（workflow_dispatch）**。本番デプロイの後に
Actions タブから Run workflow するだけ（`url` 入力で個別URLのみも可。Secret 不要）。
...
## 10. 検索サジェスト用データ（自治体の読み仮名・町丁名）

検索コンボボックス（トップFV・地図ヘッダー）を「ひらがな読み」（むなかた → 宗像市）と
「町丁名」（日の里 → 宗像市（日の里））で引けるようにするためのメタデータ。
**統計値ではない**ため honesty 方針（推計値の禁止）の対象外だが、出典は明記する。

- 出典: [Geolonia 住所データ](https://github.com/geolonia/japanese-addresses)
  （国土交通省「位置参照情報」等を元に整備・MIT License。位置参照情報の一次配布は
  フォーム経由 zip のため、機械取得できる整備版 CSV を使用）
- 生成物:
  - `data/muni-kana.json` … `{code: ひらがな読み}` 全1,918自治体。`lib/metrics.ts` が
    `MuniSummary.kana` に合流させ、クライアントのローカル絞り込みで使う。
  - `data/towns.json` … `{code: [[町丁名, ひらがな], ...]}` 約12万町丁（丁目は大字単位に
    畳んで重複排除）。**サーバー専用**（`/api/town-search` だけが読む。約4MBのため
    クライアントへは配信しない）。
...
- 元リポジトリ（japanese-addresses v1）はアーカイブ済みで CSV の更新は不定期。
  住所・読みは統計と違い変化が緩やかなので、市町村合併・政令市の区再編があった時に
  再実行すれば足りる。
- **浜松市の2024年区再編（中央区・浜名区・天竜区）が CSV 未反映**のため、浜松市の
  町丁データは未収録（読み仮名はスクリプト内 `MANUAL_KANA` で補完済み。出典は
  総務省「全国地方公共団体コード」の読み仮名欄）。北方領土6村・利島村・湯前町も
  CSV に行が無く同様に読みのみ補完。
- 政令市の親市（例 11100 さいたま市）は位置参照情報に行が無いため、区の読みの
...
## 12. 電気料金プラン（/denki シミュレーター・手動）

データ: `data/denki-plans.json`（大手10社の従量電灯=規制料金の baseline。将来は新電力 offer も追加）。
型・整合性検証は `lib/denkiPlans.ts` の `validateDenkiPlans`（`npm run test` の
`tests/lib/denkiPlans.test.ts` で実データを常時検証。`scripts/validate-data.mjs` の対象外）。

**更新頻度: 年1回の点検 + 改定の報道があったとき**。規制料金の改定は例年4月適用が多い
（直近は大手7社の2023年6月値上げ、東京・関西・九州の2024年4月改定）。燃料費調整額・再エネ
賦課金は毎月変動するが**試算に含めない設計**なので追従不要。

手順:

1. JSON 内の各プランの `sourceUrl`（各社公式の料金表ページ）を開き、基本料金・3段階単価を突合する。
2. 変わっていれば単価を更新し、`sourceAsOf`（プラン別）と `asOf`（ファイル全体、YYYY-MM）を確認日に更新する。
   `asOf` は `/denki` の sitemap `lastModified` と本文の「時点」表記に使われる。
3. `npx vitest run tests/lib/denkiPlans.test.ts` — スポットチェック（東電 30A/260kWh=9,607円、
   関電 260kWh=6,230円）は単価改定で**期待値ごと更新が必要**（公式の新単価から手計算し直す）。
4. 供給エリアの境界自治体（`lib/denki.ts` の `AREA_EXCEPTIONS`）は市町村合併でもない限り不変。
   触る場合は各エントリの `source`（送配電事業者の公式ページ）で再確認する。

既知の注意点:

- 段階の閾値に社差がある（北海道は第2段階が280kWh、四国の最低料金は11kWh・沖縄は10kWhまで）。
  閾値ごと JSON に持っているので、単価だけでなく閾値の改定にも注意。
