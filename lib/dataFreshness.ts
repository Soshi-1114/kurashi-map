...
// 元は自治体ページの再クロール停滞（2026-08 の GSC 診断で最終クロールが公開直後の
// まま7週間停止）への対策として全種別に適用したが、sitemap 2,744 URL 中 2,734 件の
// lastmod がテンプレ改訂日の同一日付に揃い、「全ページが同日に更新された」と主張する
// 毎ビルド now と同型のノイズ信号になった。lastmod はページごとに日付が分散して
// いてこそ信頼されるため、大量種別はデータ asOf 由来のみとし、テンプレ改訂の
// 通知はデプロイ後の IndexNow 送信（scripts/indexnow-submit.mjs）で代替する。
//
