...
async function loadConfig(): Promise<MetricHubConfig> {
  ...
  return {
    ...
    description: `全国1,918の市区町村・行政区の家賃相場（民営借家の家賃平均）を色分けした地図（コロプレス）。家賃が安い地域・高い地域をひと目で比較できます。都道府県別・ランキングへも展開。出典: 総務省 住宅・土地統計調査${asOfJa ? `（${asOfJa}）` : ""}。`,
    ogImage: absoluteUrl("/api/og/ranking/rent-cheap"),
    ogAlt: "家賃相場マップ",
    h1: "家賃相場を地図で見る",
    leads: [
      "全国1,918の市区町村・行政区の家賃相場（民営借家の家賃平均・円/月）を、色の濃淡で表したコロプレスマップです。色をたどるだけで、家賃が安い地域・高い地域の分布をひと目で比較できます。地図の自治体をクリックすると、その街の家賃・地価・人口推移・子育て・災害リスクなどの住環境データを確認できます。",
      `数値は総務省「住宅・土地統計調査」${asOfJa ? `（${asOfJa}）` : ""}の家賃階級別の借家数から、階級の中点で加重平均して求めた民営借家の家賃平均です（中央値ではありません）。欠損を推計で補うことはせず、住宅統計の集計対象外の小規模町村は「データなし」として灰色で表示します。`,
    ],
    ...
    foot: "© KurashiMap — 出典: 総務省 住宅・土地統計調査（e-Stat）。民営借家の家賃平均を市区町村単位で比較しています。",
    dataset: {
      name: "市区町村別 家賃相場（民営借家の家賃平均）コロプレスマップ",
      description: "総務省 住宅・土地統計調査に基づく全国1,918の市区町村・行政区の民営借家の家賃平均（円/月）のデータセット。地図上で色分け（コロプレス）して比較できる。推計値は含まない。",
