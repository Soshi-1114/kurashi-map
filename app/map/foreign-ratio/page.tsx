...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description = `全国1,918の市区町村・行政区の在留外国人の割合（％）を色分けした地図（コロプレス）。外国人が多い地域・少ない地域を視覚的に比較できます。都道府県別・ランキングへも展開。出典: 出入国在留管理庁「在留外国人統計」${asOfJa ? `（${asOfJa}）` : ""}。`;
  return {
    title,
    description,
  ...
  const ldJson = {
    ...
    "@graph": [
      ...
      },
      {
        ...
        description:
          "全国1,918の市区町村・行政区の在留外国人数と国勢調査人口から算出した、人口に占める外国人住民の割合（％）のデータセット。地図上で色分け（コロプレス）して比較できる。推計値は含まない。",
        url: absoluteUrl(PATH),
  ...
  return (
    <div className="home-links-inner">
      ...
      <p className="home-links-lead">
        全国1,918の市区町村・行政区の在留外国人の割合（人口に占める外国人住民の割合・％）を、色の濃淡で表したコロプレスマップです。色が濃い地域ほど割合が高く、外国人住民が多い地域・少ない地域をひと目で比較できます。地図の自治体をクリックすると、その街の在留外国人数・人口・人口推移などの住環境データを確認できます。
      </p>
      <p className="home-links-lead">
