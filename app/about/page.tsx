...
export default async function AboutPage() {
  ...
  return (
    <div className="detail-root">
      ...
      <section className="detail-section">
        <h2 className="detail-h2">広告・アフィリエイトについて</h2>
        <p className="detail-p">
          当サイトは、運営費用をまかない無料での提供を続けるため、一部のページに<strong>アフィリエイトプログラム（成果報酬型広告）</strong>による外部リンクを掲載することがあります。具体的には、自治体の詳細ページに設置している「ふるさと納税を見る」リンクなどが該当します。利用者がこれらのリンクを経由して寄付などを行った場合、当サイトが広告主やASP（アフィリエイト・サービス・プロバイダ）から報酬を受け取ることがあります。<strong>利用者に追加の費用が発生することはありません。</strong>
        </p>
        <p className="detail-p">
          こうした広告リンクには、成果報酬型であることがわかるよう「広告」と表示します。なお、当サイトが掲載する統計データ・スコア・ランキングは、<strong>すべて政府統計の実データに基づいており、広告主との関係や報酬の有無によって内容・順位・評価を変えることは一切ありません。</strong>データの中立性は収益化から独立して維持します。個人情報の取り扱いについては
